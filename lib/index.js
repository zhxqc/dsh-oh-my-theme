...
/** Keep only public fields from DeepSeek's balance response. */
function normalizeDeepSeekBalance(payload) {
	const infos = Array.isArray(payload?.balance_infos) ? payload.balance_infos : [];
	const balanceInfos = infos.map((info) => {
		if (info === null || typeof info !== 'object') return null;
		const currency = info.currency === 'CNY' || info.currency === 'USD' ? info.currency : null;
		const values = ['total_balance', 'granted_balance', 'topped_up_balance'];
		if (currency === null || values.some((key) => typeof info[key] !== 'string')) return null;
		return {
			currency,
				...
				members: [
					{
						kind: 'method',
						name: 'balance',
						signature: 'balance(signal): Promise<DeepSeekBalance>'
					},
					{
						kind: 'method',
						name: 'search',
...
class WorkspaceFilesRuntime extends TypertRemoteService {
	...
	/** Read the current official DeepSeek balance without exposing its API key. */
	async balance(signal) {
		const credentials = this.ctx?.get?.('credentials') ?? this.ctx?.credentials;
		if (credentials === undefined || typeof credentials.resolve !== 'function') {
			throw new Error('DeepSeek credentials service is unavailable');
		}
		const resolved = await credentials.resolve('DEEPSEEK_API_KEY');
		if (typeof resolved?.value !== 'string' || resolved.value.length === 0) {
			throw new Error('DeepSeek API key is not configured');
		}
		if (typeof fetch !== 'function') throw new Error('DeepSeek balance requires host fetch support');
		const controller = new AbortController();
		const abort = () => controller.abort();
		if (signal?.aborted) throw new Error('DeepSeek balance request was cancelled');
		...
		try {
			...
			try {
				...
				throw new Error('DeepSeek balance response was invalid');
			}
