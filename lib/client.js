	...
	factory: (require) => {
		...
		const INVOCATIONS = [
			...
			},
			{
				...
				parameters: [
					{ name: "agent", wire: "agentId", source: "lookup", lookup: "agent" },
					{ name: "relPath", wire: "relPath", source: "json" }
				],
				cancellation: { parameter: "signal" },
				result: { mode: "src-json" }
			},
		...
		/** Drawer + tree styles (all colors via --dsw-* tokens so skins apply). */
		const drawerStyles = {
			panel: {
				position: "fixed",
				left: 0,
		...
		/** The shell-overlay drawer: file tree + Markdown preview. */
		function FileTreeDrawer({ t, useScope, onClose, onToggleDir, onSelectFile }) {
			...
			return (0, react_jsx_runtime.jsxs)("div", {
				...
				children: [
					...
					(0, react_jsx_runtime.jsx)("div", {
						...
						children: [
							(0, react_jsx_runtime.jsx)("div", {
								style: drawerStyles.tree,
								children: state.sessionId === null
		...
		function FileTreeButton({ t, useScope, onToggle }) {
			const state = useScope();
		...
		function apply(ctx) {
			...
			const loadDir = async (relPath, sessionId) => {
				if (filesRemote === undefined) return;
			...
			// Keep the drawer's sessionId in sync with the current session.
			ctx.effect(() => {
			...
			}, "dsh-oh-my-theme: drawer session sync");
			...
			const drawerActions = {
				...
				onSelectFile: async (relPath) => {
					...
					try {
						...
						drawerScope.update((d) => {
							d.preview = { relative: relPath, content: result.content, truncated: result.truncated === true };
						});
					} catch (error) {
						drawerScope.update((d) => {
							d.error = error instanceof Error ? error.message : String(error);
