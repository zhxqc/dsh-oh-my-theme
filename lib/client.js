...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		function apply(ctx) {
			...
			// Hover-preview bookkeeping: the preference that a live preview must
			// restore once the pointer leaves the card. Only a click persists.
			...
			const injected = (actions) => {
				...
				return {
					setSkin: (id) => {
						// A click commits the choice and cancels any live preview.
						previewBase = null;
						ctx.theme.setTheme(id);
						writeSavedSkin(id);
					},
					previewSkin: (id) => {
						// Enter a card: remember where we came from, then switch
						// temporarily. Third-party ids never touch the durable
						// preference, so a preview cannot leak into settings.
						previewBase = ctx.theme.getTheme().preference;
						if (previewBase !== id) ctx.theme.setTheme(id);
					},
					restoreSkin: () => {
						// Leave a card: put the preference back unless a click
						// committed a new choice in the meantime.
