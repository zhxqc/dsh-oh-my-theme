...
[data-oh-my-theme-preview] h3 { font-size: calc(var(--oh-my-theme-preview-font-size, 14px) * 1.25) !important; }`;
		...
		function rewriteMarkdownImages(markdown, mdRelative, sessionId) {
			return markdown.replace(/!\[([^\]]*)\]\(([^)]+)\)/g, (match, alt, src) => {
				...
				const url = `/api/oh-my-theme/image?session=${encodeURIComponent(sessionId)}&path=${encodeURIComponent(resolved)}`;
				return `![${alt}](${url})`;
			});
		}
		...
		const drawerStyles = {
			...
			wideResizeHandle: {
				...
				width: 8, marginLeft: -4, cursor: "col-resize", touchAction: "none"
			},
			tree: {
				flex: "0 0 42%",
				minWidth: 140,
		...
		function FileSidePanel({ t, scope, onClose, onToggleDir, onSelectFile, onSetView, onOpenExternal, mode = "details" }) {
			...
			const resizeDetails = (width) => {
				const panel = panelRef.current;
				if (panel === null || mode !== "details") return 0;
				let frame = panel.parentElement;
				while (frame !== null && frame.querySelector?.(':scope > [data-shell-overlay="true"]') === null) frame = frame.parentElement;
				if (frame === null) return 0;
				const frameWidth = frame.getBoundingClientRect().width;
				const sidebarWidth = frame.firstElementChild?.getBoundingClientRect().width ?? 0;
				const maxWidth = Math.max(300, Math.min(860, frameWidth - sidebarWidth - 360));
				const next = Math.max(300, Math.min(maxWidth, Math.round(width)));
				frame.style.setProperty("grid-template-columns", `${sidebarWidth}px minmax(0, 1fr) ${next}px`, "important");
				const nativeHandle = frame.querySelector('[data-side="details"]');
				if (nativeHandle instanceof HTMLElement) nativeHandle.style.display = "none";
				panel.dataset.detailsWidth = String(next);
				return next;
			};
			(0, _react.useEffect)(() => {
				if (!state.open || mode !== "details") return;
				const savedWidth = Number(readStorage(DETAILS_WIDTH_KEY));
				if (Number.isFinite(savedWidth) && savedWidth >= 300) resizeDetails(savedWidth);
			}, [state.open, mode]);
			const startResize = (event) => {
				...
				const move = (moveEvent) => resizeDetails(startWidth + startX - moveEvent.clientX);
				const end = (upEvent) => {
					const width = resizeDetails(startWidth + startX - upEvent.clientX);
			...
			return (0, react_jsx_runtime.jsxs)("div", {
				...
				children: [
					mode === "details" && (0, react_jsx_runtime.jsx)("div", {
						role: "separator",
