...
//  4. Live DeepSeek balance — a compact readout below the composer stats
//     uses the host-only credentials service and the official balance API.
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		const BALANCE_REFRESH_LABEL = "balance.refresh";
		...
		const zh = {
			...
			"balance.unconfigured": "未配置 DEEPSEEK_API_KEY",
			"balance.hostOutdated": "宿主插件尚未更新，请重启 DSH 或重新安装插件",
			"balance.requestFailed": "余额暂时不可用，请稍后重试",
			"balance.empty": "暂无余额信息",
			"balance.checkedAt": "更新于",
		...
		const en = {
			...
			"balance.unavailable": "Insufficient balance",
			"balance.granted": "Granted",
			"balance.toppedUp": "Topped up",
			"balance.refresh": "Refresh balance",
			"balance.loading": "Reading balance…",
			"balance.unconfigured": "DEEPSEEK_API_KEY is not configured",
			"balance.hostOutdated": "The host plugin is outdated. Restart DSH or reinstall the plugin",
			"balance.requestFailed": "Balance is temporarily unavailable. Try again later",
			"balance.empty": "No balance information",
			"balance.checkedAt": "Updated",
...
[data-oh-my-theme-preview] h3 { font-size: calc(var(--oh-my-theme-preview-font-size, 14px) * 1.25) !important; }`;
		...
		const INVOCATIONS = [
			{
				id: "dsh-oh-my-theme#workspaceFiles/balance",
				...
				result: strictCodec("dsh-oh-my-theme#DeepSeekBalance")
			},
			{
		...
		/** Format a balance string without losing the provider's decimal precision. */
		function formatBalanceAmount(value, currency) {
			const symbol = currency === "CNY" ? "¥" : "$";
			return `${symbol}${value}`;
		}

		function balanceErrorLabel(error, t) {
			if (error.includes("not configured")) return t("balance.unconfigured");
			if (error.includes("HTTP 404") || error.includes("workspaceFiles/balance")) return t("balance.hostOutdated");
			return t("balance.requestFailed");
		}

		/** Compact balance readout for the composer dock with a detail popover. */
		function BalanceButton({ t, scope, onToggle, onRefresh, dock = false }) {
			const state = (0, _react.useSyncExternalStore)(scope.subscribe, scope.getSnapshot);
			const rootRef = (0, _react.useRef)(null);
			(0, _react.useEffect)(() => {
				if (!state.open || typeof document === "undefined" || typeof document.addEventListener !== "function") return;
				const onPointerDown = (event) => {
					if (!rootRef.current?.contains?.(event.target)) onToggle();
				};
				document.addEventListener("pointerdown", onPointerDown, true);
				return () => document.removeEventListener("pointerdown", onPointerDown, true);
			}, [state.open, onToggle]);
			const data = state.data;
			const statusLabel = data === null ? null : (data.is_available ? t("balance.available") : t("balance.unavailable"));
			const summary = data?.balance_infos?.map((info) => formatBalanceAmount(info.total_balance, info.currency)).join(" · ")
				?? (state.loading ? t("balance.loading") : t("balance.title"));
			return (0, react_jsx_runtime.jsxs)("div", {
				ref: rootRef,
				style: dock ? drawerStyles.balanceDock : drawerStyles.balanceWrap,
				children: [
					(0, react_jsx_runtime.jsx)(_ui_primitives.Tooltip, {
						...
						children: (0, react_jsx_runtime.jsx)("button", {
							...
							style: dock ? drawerStyles.balanceDockButton : drawerStyles.balanceToggle,
							children: [
								(0, react_jsx_runtime.jsx)(_ui_primitives.IconDataOutline16, { size: 16 }),
								dock && (0, react_jsx_runtime.jsx)("span", { children: summary })
							]
						})
					}),
					state.open && (0, react_jsx_runtime.jsxs)("div", {
						role: "dialog",
						"aria-label": t("balance.title"),
						style: dock ? { ...drawerStyles.balancePopover, ...drawerStyles.balanceDockPopover } : drawerStyles.balancePopover,
						children: [
							(0, react_jsx_runtime.jsxs)("div", {
								style: drawerStyles.balanceHeader,
								children: [
									(0, react_jsx_runtime.jsx)("span", { children: t("balance.title") }),
									(0, react_jsx_runtime.jsx)("button", {
										type: "button",
										onClick: onRefresh,
										disabled: state.loading,
										"aria-label": t(BALANCE_REFRESH_LABEL),
										title: t(BALANCE_REFRESH_LABEL),
										style: drawerStyles.balanceRefresh,
										children: (0, react_jsx_runtime.jsx)(_ui_primitives.IconRefreshOutline16, { size: 14 })
									})
								]
							}),
							state.loading && (0, react_jsx_runtime.jsx)("div", { style: drawerStyles.balanceStatus, children: t("balance.loading") }),
							state.error !== null && (0, react_jsx_runtime.jsx)("div", { style: drawerStyles.error, children: balanceErrorLabel(state.error, t) }),
							statusLabel !== null && (0, react_jsx_runtime.jsx)("div", {
								style: { ...drawerStyles.balanceStatus, ...(data.is_available ? drawerStyles.balanceStatusGood : drawerStyles.balanceStatusBad) },
								children: statusLabel
							}),
							data?.balance_infos?.length > 0
								? data.balance_infos.map((info) => (0, react_jsx_runtime.jsxs)("div", {
									style: drawerStyles.balanceRow,
									children: [
										(0, react_jsx_runtime.jsx)("span", { style: drawerStyles.balanceLabel, children: info.currency }),
										(0, react_jsx_runtime.jsx)("strong", { style: drawerStyles.balanceAmount, children: formatBalanceAmount(info.total_balance, info.currency) }),
										(0, react_jsx_runtime.jsx)("span", { style: drawerStyles.balanceLabel, children: t("balance.granted") }),
										(0, react_jsx_runtime.jsx)("span", { style: drawerStyles.balanceAmount, children: formatBalanceAmount(info.granted_balance, info.currency) }),
										(0, react_jsx_runtime.jsx)("span", { style: drawerStyles.balanceLabel, children: t("balance.toppedUp") }),
										(0, react_jsx_runtime.jsx)("span", { style: drawerStyles.balanceAmount, children: formatBalanceAmount(info.topped_up_balance, info.currency) })
									]
								}, info.currency))
								: data !== null && state.error === null && !state.loading
									? (0, react_jsx_runtime.jsx)("div", { style: drawerStyles.balanceStatus, children: t("balance.empty") })
									: null,
							data?.fetched_at !== undefined && (0, react_jsx_runtime.jsx)("div", {
								style: drawerStyles.balanceFooter,
								children: `${t("balance.checkedAt")} ${new Date(data.fetched_at).toLocaleTimeString()}`
							})
						]
					})
				]
		...
		function apply(ctx) {
			...
			let filesRemote;
			const balanceScope = (0, _runtime_client.createSnapshotStore)({
				open: false,
				loading: false,
				data: null,
				error: null
			...
			const loadBalance = async () => {
				if (filesRemote === undefined || balanceScope.getSnapshot().loading) return;
				balanceScope.update((d) => {
					d.loading = true;
					d.error = null;
				});
				try {
					const data = await callRemote("balance", []);
					balanceScope.update((d) => {
						d.data = data;
						d.loading = false;
					});
			...
			const toggleBalance = () => {
				const open = !balanceScope.getSnapshot().open;
				balanceScope.update((d) => { d.open = open; });
				if (open && balanceScope.getSnapshot().data === null) void loadBalance();
			};
