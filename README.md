...
> 点击 Session log 左侧的工作区按钮打开。可切换「仅项目文件 / 文件与预览分栏 / 仅文件预览」，Markdown 支持语法高亮。

- **懒加载文件树** —— 目录按需逐层加载，自动跳过 `node_modules` / `.git` / 构建产物目录；文件树与 `@` 菜单共用 VSCode Material Icon Theme 图标。
- **独立视图** —— 面板标题栏可切换「仅项目文件 / 分栏 / 仅文件预览」三种布局。
- **Markdown 预览** —— `.md` 使用 dsh 基于 Shiki 的 Markdown 渲染器，围栏代码支持语法高亮和复制按钮；其他 UTF-8 文本以纯文本展示（512KB 上限、二进制拒绝读取）。
