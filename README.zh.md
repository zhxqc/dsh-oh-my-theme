...
3. **右侧文件面板** —— Codex 风格：右侧边缘的项目文件树（左侧）与文件预览（右侧），Markdown 渲染、其他文本文件以纯文本展示。
...
- **文件预览** —— 点击文件在面板右栏打开：`.md` 用共享 Markdown 组件渲染，其他 UTF-8 文本以纯文本展示（512KB 上限、二进制拒绝读取）。
- **中英双语** —— 所有界面跟随 GUI 语言。
...
5. **挂载右侧文件面板** —— `sidebar.footer.action` 的开关按钮 + root 作用域 `shell.overlay` 列表槽里的面板，固定在右侧边缘（overlay 层是 `pointer-events: none`、子项 auto，面板可交互而页面其余部分保持可点）。两个槽共享同一个快照 store，跟随 `sessions.list` 的当前会话，并把远程就绪状态发布到界面，加载失败可见。
...
点击侧边栏底部（左栏最下方）的**文件夹**按钮，在右侧边缘打开 Codex 风格面板。左栏是懒加载的项目文件树；点击文件在右栏打开预览 —— `.md` 用共享 Markdown 组件渲染，其他文本文件以纯文本展示。面板始终跟随当前打开会话的工作区。
